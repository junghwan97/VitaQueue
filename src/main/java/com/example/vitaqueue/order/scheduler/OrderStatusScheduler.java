package com.example.vitaqueue.order.scheduler;
...
public class OrderStatusScheduler {
    ...
    public void updateOrderStatuses() {
        ...
        List<OrderEntity> toDeliveryOrders = orderRepository.findAllByStatusAndRegisteredAtBefore(OrderStatus.CREATED, new Timestamp(now.getTime()));
        // 주문 상태 변경
        toDeliveryOrders.forEach(order -> {order.updateStatus(OrderStatus.DELIVERY);});
        orderRepository.saveAll(toDeliveryOrders);
        // 상세 주문 상태 변경
        List<OrderProductEntity> toDeliveryOrderProducts = orderProductRepository.findAllByStatusAndRegisteredAtBefore(OrderStatus.CREATED, new Timestamp(now.getTime()));
        ...
        // 상세 주문 상태 변경
        List<OrderProductEntity> toDeliveredOrderProducts = orderProductRepository.findAllByStatusAndRegisteredAtBefore(OrderStatus.CREATED, new Timestamp(now.getTime() - 24 * 60 * 60 * 1000));
